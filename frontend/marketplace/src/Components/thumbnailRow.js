...
const ThumbnailRow = ({ images }) => {
  const renderThumbnails = () => {
    const visibleThumbnails = images.slice(0, 6); // Display up to 6 thumbnails
  ...
  const remainingThumbnailsCount = images.length - 6; // Exclude the main image and the first 6 thumbnails
